...
type HomePatientItem = { patient: PatientWithGestationalInfo; teamMembers: TeamMember[] };
...
function getCachedEnterpriseHomePatients(params: FetchParams): Promise<HomePatientItem[]> {
...
export const getEnterpriseHomePatientsAction = authActionClient
  ...
  .action(async ({ parsedInput, ctx: { profile } }) => {
    if (!profile.enterprise_id) {
      return { items: [] as HomePatientItem[] };
    }
